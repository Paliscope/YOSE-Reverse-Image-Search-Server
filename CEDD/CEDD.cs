...
namespace CEDD_Descriptor
{
    public  class CEDD
    {
            ...
            for (int y = 0; y < TemoMAX_Y; y += Step_Y)
            {
                ...
                for (int x = 0; x < TemoMAX_X; x += Step_X)
                {
                   ...
                   TempSum = 0;
                    ...
                    for (int i = y; i < Math.Min(y + Step_Y, height); i ++) // + Step_Y
                    {
                        for (int j = x; j <Math.Min(x +Step_X, width) ; j++) //+ Step_X
                        {
                            // Color Information
        ...
        //Compare CEDD Discriptor using TanimotoClassifier algorimthm
        public static float Compare2(float[] Table1, float[] Table2)
        {
            float Result = 0;
            float Temp1 = 0;
            float Temp2 = 0;
        ...
        //Compare CEDD Discriptor using TanimotoClassifier algorimthm
        public static float Compare2(ushort[] Table1, ushort[] Table2, float Temp1, float Temp2)
        {
            ...
            if (Temp1 > 0 && Temp2 > 0)
            {
                ...
                return (100 - 100 * (TempCount1 / (TempCount2 + TempCount3 - TempCount1))); //Tanimoto
            }
            return 0;
        }
